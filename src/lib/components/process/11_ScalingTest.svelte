<script>
	...
	let rhombusLength = $state(1); // Skalierungsfaktor für die Länge der Rauten (0.5 bis 2)
	let scaledSquareSize = $derived(squareSize * rhombusLength);
	let Parallelogramm = $derived(
		`0,0 ${scaledSquareSize},0 ${scaledSquareSize + cosSize},${cosSize} ${cosSize},${cosSize}`
	);
	let Rotatepoint = $derived((scaledSquareSize + cosSize) / 2);
	let offset = $state(0);

	function calculatePosition(xi, yi) {
		const x = xi * (scaledSquareSize + cosSize + offset);
		const y = yi * (scaledSquareSize + cosSize + offset);
...
<div id="control3">
	<input type="range" min="0.5" max="2" step="0.1" bind:value={rhombusLength} />
</div>
